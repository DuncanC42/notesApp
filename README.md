# Notes App

## Roadmap

1. Setup NPM project
```sh
npm init -y
npm i express@4.19.2 mongoose@8.5.1 body-parser@1.20.2 --save-exact
npm i --save-exact --save-dev nodemon@3.1.4
```

2. Dockerize backend
3. Setup docker-compose for Notebooks services
4. Setup docker-compose for Notes services
5. Use multistage build for our images
6. Merging multiple projects in docker compose
7. Implement Nginx reverse proxy
8. Establish connection between services
9. Setup Models and Routes for the Notebooks service
10. Implement Routes to create and retrieve Notebooks
11. Implement Routes to to update and delete Notebooks
12. Implement Routes and business logic in the notes backend
13. Storing Notebooks IDs in the Notes Service
